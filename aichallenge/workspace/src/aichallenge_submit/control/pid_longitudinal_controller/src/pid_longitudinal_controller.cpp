...
: node_{&node}, diagnostic_updater_(&node)
{
  ...
  // parameters for state transition
  {
    ...
    // drive
    p.drive_state_stop_dist = node_->declare_parameter<double>("pid_drive_state_stop_dist");  // [m]
    p.drive_state_offset_stop_dist =
      node_->declare_parameter<double>("drive_state_offset_stop_dist");  // [m]
    // stopping
    p.stopping_state_stop_dist =
      node_->declare_parameter<double>("pid_stopping_state_stop_dist");  // [m]
    p.stopped_state_entry_duration_time =
      node_->declare_parameter<double>("pid_stopped_state_entry_duration_time");  // [s]
    // stop
    p.stopped_state_entry_vel =
      node_->declare_parameter<double>("pid_stopped_state_entry_vel");  // [m/s]
    p.stopped_state_entry_acc =
      node_->declare_parameter<double>("pid_stopped_state_entry_acc");  // [m/s²]

    // emergency
    p.emergency_state_overshoot_stop_dist =
      node_->declare_parameter<double>("pid_emergency_state_overshoot_stop_dist");  // [m]
    p.emergency_state_traj_trans_dev =
      node_->declare_parameter<double>("pid_emergency_state_traj_trans_dev");  // [m]
    p.emergency_state_traj_rot_dev =
      node_->declare_parameter<double>("pid_emergency_state_traj_rot_dev");  // [m]
  }

  // parameters for drive state
  {
    // initialize PID gain
    const double kp{node_->declare_parameter<double>("pid_kp")};
    const double ki{node_->declare_parameter<double>("pid_ki")};
    const double kd{node_->declare_parameter<double>("pid_kd")};
    m_pid_vel.setGains(kp, ki, kd);

    // initialize PID limits
    const double max_pid{node_->declare_parameter<double>("pid_max_out")};     // [m/s^2]
    const double min_pid{node_->declare_parameter<double>("pid_min_out")};     // [m/s^2]
    const double max_p{node_->declare_parameter<double>("pid_max_p_effort")};  // [m/s^2]
    const double min_p{node_->declare_parameter<double>("pid_min_p_effort")};  // [m/s^2]
    const double max_i{node_->declare_parameter<double>("pid_max_i_effort")};  // [m/s^2]
    const double min_i{node_->declare_parameter<double>("pid_min_i_effort")};  // [m/s^2]
    const double max_d{node_->declare_parameter<double>("pid_max_d_effort")};  // [m/s^2]
    const double min_d{node_->declare_parameter<double>("pid_min_d_effort")};  // [m/s^2]
    ...
    m_current_vel_threshold_pid_integrate =
      node_->declare_parameter<double>("pid_current_vel_threshold_pid_integration");  // [m/s]
  ...
  // parameters for smooth stop state
  {
    const double max_strong_acc{
      node_->declare_parameter<double>("pid_smooth_stop_max_strong_acc")};  // [m/s^2]
    const double min_strong_acc{
      node_->declare_parameter<double>("pid_smooth_stop_min_strong_acc")};                // [m/s^2]
    const double weak_acc{node_->declare_parameter<double>("pid_smooth_stop_weak_acc")};  // [m/s^2]
    const double weak_stop_acc{
      node_->declare_parameter<double>("pid_smooth_stop_weak_stop_acc")};  // [m/s^2]
    const double strong_stop_acc{
      node_->declare_parameter<double>("pid_smooth_stop_strong_stop_acc")};  // [m/s^2]

    const double max_fast_vel{
      node_->declare_parameter<double>("pid_smooth_stop_max_fast_vel")};  // [m/s]
    const double min_running_vel{
      node_->declare_parameter<double>("pid_smooth_stop_min_running_vel")};  // [m/s]
    const double min_running_acc{
      node_->declare_parameter<double>("pid_smooth_stop_min_running_acc")};  // [m/s^2]
    const double weak_stop_time{
      node_->declare_parameter<double>("pid_smooth_stop_weak_stop_time")};  // [s]

    const double weak_stop_dist{
      node_->declare_parameter<double>("pid_smooth_stop_weak_stop_dist")};  // [m]
    const double strong_stop_dist{
      node_->declare_parameter<double>("pid_smooth_stop_strong_stop_dist")};  // [m]
  ...
  // parameters for stop state
  {
    ...
    p.vel = node_->declare_parameter<double>("pid_stopped_vel");    // [m/s]
    p.acc = node_->declare_parameter<double>("pid_stopped_acc");    // [m/s^2]
    p.jerk = node_->declare_parameter<double>("pid_stopped_jerk");  // [m/s^3]
  }

  // parameters for emergency state
  {
    auto & p = m_emergency_state_params;
    p.vel = node_->declare_parameter<double>("pid_emergency_vel");    // [m/s]
    p.acc = node_->declare_parameter<double>("pid_emergency_acc");    // [m/s^2]
    p.jerk = node_->declare_parameter<double>("pid_emergency_jerk");  // [m/s^3]
  }

  // parameters for acceleration limit
  m_max_acc = node_->declare_parameter<double>("pid_max_acc");  // [m/s^2]
  m_min_acc = node_->declare_parameter<double>("pid_min_acc");  // [m/s^2]

  // parameters for jerk limit
  m_max_jerk = node_->declare_parameter<double>("pid_max_jerk");  // [m/s^3]
  m_min_jerk = node_->declare_parameter<double>("pid_min_jerk");  // [m/s^3]
  ...
  m_max_pitch_rad = node_->declare_parameter<double>("pid_max_pitch_rad");  // [rad]
  m_min_pitch_rad = node_->declare_parameter<double>("pid_min_pitch_rad");  // [rad]
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  // delay compensation
  update_param("pid_delay_compensation_time", m_delay_compensation_time);

  // state transition
  {
    auto & p = m_state_transition_params;
    update_param("pid_drive_state_stop_dist", p.drive_state_stop_dist);
    update_param("pid_stopping_state_stop_dist", p.stopping_state_stop_dist);
    update_param("pid_stopped_state_entry_duration_time", p.stopped_state_entry_duration_time);
    update_param("pid_stopped_state_entry_vel", p.stopped_state_entry_vel);
    update_param("pid_stopped_state_entry_acc", p.stopped_state_entry_acc);
    update_param("pid_emergency_state_overshoot_stop_dist", p.emergency_state_overshoot_stop_dist);
    update_param("pid_emergency_state_traj_trans_dev", p.emergency_state_traj_trans_dev);
    update_param("pid_emergency_state_traj_rot_dev", p.emergency_state_traj_rot_dev);
  }

  // drive state
  {
    double kp{node_->get_parameter("pid_kp").as_double()};
    double ki{node_->get_parameter("pid_ki").as_double()};
    double kd{node_->get_parameter("pid_kd").as_double()};
    update_param("pid_kp", kp);
    update_param("pid_ki", ki);
    update_param("pid_kd", kd);
    m_pid_vel.setGains(kp, ki, kd);

    double max_pid{node_->get_parameter("pid_max_out").as_double()};
    double min_pid{node_->get_parameter("pid_min_out").as_double()};
    double max_p{node_->get_parameter("pid_max_p_effort").as_double()};
    double min_p{node_->get_parameter("pid_min_p_effort").as_double()};
    double max_i{node_->get_parameter("pid_max_i_effort").as_double()};
    double min_i{node_->get_parameter("pid_min_i_effort").as_double()};
    double max_d{node_->get_parameter("pid_max_d_effort").as_double()};
    double min_d{node_->get_parameter("pid_min_d_effort").as_double()};
    update_param("pid_max_out", max_pid);
    update_param("pid_min_out", min_pid);
    update_param("pid_max_p_effort", max_p);
    update_param("pid_min_p_effort", min_p);
    update_param("pid_max_i_effort", max_i);
    update_param("pid_min_i_effort", min_i);
    update_param("pid_max_d_effort", max_d);
    update_param("pid_min_d_effort", min_d);
    m_pid_vel.setLimits(max_pid, min_pid, max_p, min_p, max_i, min_i, max_d, min_d);

    update_param("pid_current_vel_threshold_pid_integration", m_current_vel_threshold_pid_integrate);
  }

  // stopping state
  {
    double max_strong_acc{node_->get_parameter("pid_smooth_stop_max_strong_acc").as_double()};
    double min_strong_acc{node_->get_parameter("pid_smooth_stop_min_strong_acc").as_double()};
    double weak_acc{node_->get_parameter("pid_smooth_stop_weak_acc").as_double()};
    double weak_stop_acc{node_->get_parameter("pid_smooth_stop_weak_stop_acc").as_double()};
    double strong_stop_acc{node_->get_parameter("pid_smooth_stop_strong_stop_acc").as_double()};
    double max_fast_vel{node_->get_parameter("pid_smooth_stop_max_fast_vel").as_double()};
    double min_running_vel{node_->get_parameter("pid_smooth_stop_min_running_vel").as_double()};
    double min_running_acc{node_->get_parameter("pid_smooth_stop_min_running_acc").as_double()};
    double weak_stop_time{node_->get_parameter("pid_smooth_stop_weak_stop_time").as_double()};
    double weak_stop_dist{node_->get_parameter("pid_smooth_stop_weak_stop_dist").as_double()};
    double strong_stop_dist{node_->get_parameter("pid_smooth_stop_strong_stop_dist").as_double()};
    update_param("pid_smooth_stop_max_strong_acc", max_strong_acc);
    update_param("pid_smooth_stop_min_strong_acc", min_strong_acc);
    update_param("pid_smooth_stop_weak_acc", weak_acc);
    update_param("pid_smooth_stop_weak_stop_acc", weak_stop_acc);
    update_param("pid_smooth_stop_strong_stop_acc", strong_stop_acc);
    update_param("pid_smooth_stop_max_fast_vel", max_fast_vel);
    update_param("pid_smooth_stop_min_running_vel", min_running_vel);
    update_param("pid_smooth_stop_min_running_acc", min_running_acc);
    update_param("pid_smooth_stop_weak_stop_time", weak_stop_time);
    update_param("pid_smooth_stop_weak_stop_dist", weak_stop_dist);
    update_param("pid_smooth_stop_strong_stop_dist", strong_stop_dist);
    m_smooth_stop.setParams(
      max_strong_acc, min_strong_acc, weak_acc, weak_stop_acc, strong_stop_acc, max_fast_vel,
      min_running_vel, min_running_acc, weak_stop_time, weak_stop_dist, strong_stop_dist);
  }
  ...
  {
    auto & p = m_stopped_state_params;
    update_param("pid_stopped_vel", p.vel);
    update_param("pid_stopped_acc", p.acc);
    update_param("pid_stopped_jerk", p.jerk);
  }

  // emergency state
  {
    auto & p = m_emergency_state_params;
    update_param("pid_emergency_vel", p.vel);
    update_param("pid_emergency_acc", p.acc);
    update_param("pid_emergency_jerk", p.jerk);
  }

  // acceleration limit
  update_param("pid_min_acc", m_min_acc);

  // jerk limit
  update_param("pid_max_jerk", m_max_jerk);
  update_param("pid_min_jerk", m_min_jerk);

  // slope compensation
  update_param("pid_max_pitch_rad", m_max_pitch_rad);
  update_param("pid_min_pitch_rad", m_min_pitch_rad);
